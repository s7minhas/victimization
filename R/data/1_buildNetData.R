...
summStatsACLED_toFocus = data.frame(do.call('rbind', lapply(toFocus, function(c){
	do.call('rbind', lapply(years, function(t){
		...
		cntActors = length(unique(c(slice$ACTOR1, slice$ACTOR2)))
		cntDyads = length(unique(slice$dyad_name))
		c(cntry=c, year=t, cntConf=cntConf, cntActors=cntActors, cntDyads=cntDyads)
	}) )
})), stringsAsFactors=FALSE)

for(v in c('year','cntConf','cntActors','cntDyads')){ summStatsACLED_toFocus[,v]=num(summStatsACLED_toFocus[,v])}

g=ggplot(summStatsACLED_toFocus, aes(x=year, y=cntConf, group=1)) +
	geom_line() +
	geom_point() +
...
ggsave(g, file=paste0(pathDrop, 'cntConf.pdf'), width=9, height=5)

g=ggplot(summStatsACLED_toFocus, aes(x=year, y=cntActors, group=1)) +
	geom_line() +
	geom_point() +
...
ggsave(g, file=paste0(pathDrop, 'cntActors.pdf'), width=9, height=5)

g=ggplot(summStatsACLED_toFocus, aes(x=year, y=cntDyads, group=1)) +
	geom_line() +
	geom_point() +
...
ggsave(g, file=paste0(pathDrop, 'cntDyads.pdf'), width=9, height=5)