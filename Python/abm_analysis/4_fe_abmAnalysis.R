...
if(!file.exists(paste0(pathResults, 'abm_feMods.rda'))){
...
save(coefs,
	...
	load(paste0(pathResults, 'abm_feCoefs.rda'))
}
################################################

################################################
# viz of results
raw = coefs$graph_dens[1:3,]
coefData = raw %>%
...
ggsave(ggCoef,
	width=7, height=4,
	file=paste0(pathGraphics, 'abm_fe_coefPlot.pdf'),
	device=cairo_pdf
	)
################################################
